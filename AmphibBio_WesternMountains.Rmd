...
These are the packages and their libraries.
...
## BiodiversityR requires the following:

install.packages(pkgs=c("BiodiversityR", "vegan","Rcmdr", "MASS", "mgcv","cluster", "RODBC", "rpart", "effects", "multcomp","ellipse", "maptree", "sp", "splancs", "spatial","akima", "nnet", "dismo", "raster", "rgdal",
"bootstrap", "PresenceAbsence","maxlike", "gbm", "randomForest", "gam", "earth", "mda","kernlab", "e1071", "glmnet", "sem", "rgl", "relimp","lmtest", "leaps", "Hmisc", "colorspace", "aplpack","abind", "XLConnect", "car", "markdown", "knitr","geosphere", "maptools", "rgeos", "ENMeval", "red"),dependencies=c("Depends", "Imports"))
...
```
# Nak'azdli Whut'en Territory

In this chunk I loaded a shapefile of Nak'azdli Whut'en territory to calculate disturbance statistics for this part of the study area. The first part looks into the planetary computer catalogue of 10m Annual Land Use Land Cover (9-class) V2 (https://planetarycomputer.microsoft.com/dataset/io-lulc-annual-v02). I reduced in size to the Nechako Lowlands to help with the processing speed and to focus in on an area that I am more familiar with from field work and research I have done locally on amphibian populations in forest harvest areas. The average model prediction accuracy for this product is 75% and it does not do an effective job of identifying harvest areas.

Next, a Landsat product (Zhang et al. 2023) is studied for the entire Nak'azdli Whut'en territory. This is an interesting product that has had a lot of model checks with global validation sets. The paper goes into further details with the key conclusion that this product can be used to model trends in land cover change with >80% accuracy depending on the land cover type, some that are easier to detect. The fine classification value IDs are listed in Zhang et al. (2021) and the authors use a nine classification system as follows:

This code adapts the fine into the nine by classifying a range:

Range 10-20 = CRP: cropland
10	Rainfed cropland
11	Herbaceous cover
12	Tree or shrub cover (Orchard)
20	Irrigated cropland

Range 50-90 = FST: forest
...
Range 180 = WET: wetlands
180	Wetlands

Range 190 = IMP: impervious surfaces;
190	Impervious

Range 200-202 = BaA: bare areas;
200	Bare areas
201	Consolidated bare areas
202	Unconsolidated bare areas

Range 210 = Wat: water body;
210	Water body

Range 220-250 = PIS: permanent ice and snow.
220	Permanent ice and snow
250	Filled value


##References
Zhang, X., Liu, L., Chen, X., Gao, Y., Xie, S., & Mi, J. (2021). GLC_FCS30: Global land-cover product with fine classification system at 30&thinsp;m using time-series Landsat imagery. Earth System Science Data, 13(6), 2753–2776. https://doi.org/10.5194/essd-13-2753-2021

Zhang, X., Zhao, T., Xu, H., Liu, W., Wang, J., Chen, X., & Liu, L. (2023). GLC_FCS30D: The first global 30-m land-cover dynamic monitoring product with a fine classification system from 1985 to 2022 using dense time-series Landsat imagery and continuous change-detection method. Earth System Science Data Discussions, 1–32. https://doi.org/10.5194/essd-2023-320


```{r Nakazdli, echo=FALSE, eval=FALSE}

Nakazdli <- vect(paste0(here("Inputs","Shapefiles"),
                        "/Nakazdli_Territory.shp"))

Nakazdli <- terra::project(Nakazdli,
                          "EPSG:4326")
Nakazdli_sf <- sf::st_as_sf(Nakazdli)

ECOSEC <- vect(paste0(here("Inputs",
                           "Shapefiles",
                           "ERC_ECOSECTIONS_SP"),
                      "/ERC_ECOSEC_polygon.shp"))

# Select the Nechako Lowland Ecosection:

ECOSEC_NEL <- ECOSEC[3]
ECOSEC_NEL  <- terra::project(ECOSEC_NEL,
                          "EPSG:4326")
ECOSEC_NEL_c <- crop(ECOSEC_NEL, Nakazdli)
ECOSEC_NEL_c_sf <- sf::st_as_sf(ECOSEC_NEL_c)
ECOSEC_NEL_ext <- as.numeric(as.matrix(ext(ECOSEC_NEL_c_sf)))
...
S2LC9V2 <- get_stac_data(
  ...
  output_filename = paste0(here::here("Outputs","Rasters"),"/S2LC9V2_x.tif")
)

S2LC9V2 <- dir(paste0(here::here("Outputs","Rasters")),
               full.names=TRUE,
               pattern="*\\d.tif")

S2LC9V2 <- lapply(S2LC9V2, rast)
S2LC9V2_f = lapply(S2LC9V2,as.factor)

S2LC9V2_sds <- terra::sds(S2LC9V2_f)

citlist <- vector(mode='list',length=length(S2LC9V2_sds))
for(i in 1:length(S2LC9V2_sds)) {
        citlist[[i]] <- crop(S2LC9V2_sds[i], ECOSEC_NEL, mask=TRUE)
    }

S2LC9V2_sds <- terra::sds(citlist)

citlist <- vector(mode='list',length=length(S2LC9V2_sds))
for(i in 1:length(S2LC9V2_sds)) {
        citlist[[i]] <- crop(S2LC9V2_sds[i], Nakazdli, mask=TRUE)
    }
...
S2LC9V2_17f <- freq(S2LC9V2_sds[1])
S2LC9V2_18f <- freq(S2LC9V2_sds[2])
S2LC9V2_19f <- freq(S2LC9V2_sds[3])
S2LC9V2_20f <- freq(S2LC9V2_sds[4])
S2LC9V2_21f <- freq(S2LC9V2_sds[5])
S2LC9V2_22f <- freq(S2LC9V2_sds[6])
S2LC9V2_23f <- freq(S2LC9V2_sds[7])

Distp <- list()

#Trees lost

Distp[1] <- S2LC9V2_17f[11,3]
Distp[2] <- S2LC9V2_18f[11,3]
Distp[3] <- S2LC9V2_19f[11,3]
Distp[4] <- S2LC9V2_20f[11,3]
Distp[5] <- S2LC9V2_21f[11,3]
Distp[6] <- S2LC9V2_22f[11,3]
Distp[7] <- S2LC9V2_23f[11,3]

Distp <- as.data.frame(unlist(Distp))
Distp$year <- c(2017:2023) 

## Landsat class
# https://zenodo.org/records/8239305

GLC_FCS30D <- c(paste0(here("Inputs","Rasters","Nak'azdli"), "/GLC_FCS30D_20002022_W125N60_Annual.tif"),paste0(here("Inputs","Rasters","Nak'azdli"), "/GLC_FCS30D_20002022_W130N60_Annual.tif"),paste0(here("Inputs","Rasters","Nak'azdli"), "/GLC_FCS30D_20002022_W130N55_Annual.tif"),paste0(here("Inputs","Rasters","Nak'azdli"), "/GLC_FCS30D_20002022_W125N55_Annual.tif"))

GLC_FCS30Dr <- vrt(GLC_FCS30D)


citlist <- vector(mode='list',length=length(GLC_FCS30D))
for(i in 1:length(names(GLC_FCS30Dr))){
        citlist[[i]] <- crop(GLC_FCS30Dr[[i]], Nakazdli, mask=TRUE)
    }

GLC_FCS30D_sds <- terra::sds(citlist)

GLC_FCS30D_sds_f = lapply(GLC_FCS30D_sds,as.factor)

#1 Range 10-20 = CRP: cropland
#2 Range 50-92 = FST: forest
#3 Range 120-153 = SHR: shrubland
#4 Range 180 = WET: wetlands
#5 Range 190 = IMP: impervious surfaces;
#6 Range 200-202 = BaA: bare areas;
#7 Range 210 = Wat: water body;
#8 Range 220-250 = PIS: permanent ice and snow.

cat_mat <- matrix(nrow=8, ncol=3)
cat_mat[1,] <- c(0, 20, 1)
cat_mat[2,] <- c(50, 92, 2)
cat_mat[3,] <- c(119, 153, 3)
cat_mat[4,] <- c(180, 187, 4)
cat_mat[5,] <- c(189, 190, 5)
cat_mat[6,] <- c(199, 202, 6)
cat_mat[7,] <- c(209, 210, 7)
cat_mat[8,] <- c(219, 250, 8)

GLC_FCS30D_sds_fc <- lapply(GLC_FCS30D_sds_f,
                            function(x){classify(x,
                                                 cat_mat,
                                                 include.lowest=TRUE,
                                                 brackets=TRUE)})
GLC_FCS30D_sds_fc <- lapply(GLC_FCS30D_sds_fc,as.factor)
GLC_FCS30D_sds_fc <- sds(GLC_FCS30D_sds_fc)

citlist <- vector(mode='list',length=length(GLC_FCS30D_sds_fc))
for(i in 1:length(names(GLC_FCS30D_sds_fc))){
        citlist[[i]] <- terra::project(GLC_FCS30D_sds_fc[i],
                                       "epsg:32610",
                                       threads=n.cores)
    }

GLC_FCS30D_sds_fc <- terra::sds(citlist)

years <- c(2000:2022)
class_stat <- list()
for(i in 1:length(GLC_FCS30D_sds_fc)){
  class_stat[[i]] <- cbind(freq(GLC_FCS30D_sds_fc[i]),Year=years[i])
}

class_stat <- as.data.frame(data.table::rbindlist(class_stat))

ctypes <- as.data.frame(cbind(value=as.numeric(c(1:8)),
                              ctype=c("CRP",
                                      "FST",
                                      "SHR",
                                      "WET",
...
classnames <- as.data.frame(cbind(ctype=c("CRP",
                            "FST",
                            "SHR",
                            "WET",
                            "IMP",
...
class_stat <- merge(class_stat,ctypes, by = "value")
class_stat <- class_stat[,c(4,5,3)]

class_stat <- class_stat[order(class_stat[,1], class_stat[,2]), ]
rownames(class_stat) <- 1:nrow(class_stat)

class_stat <- merge(class_stat,classnames, by = "ctype")

# Each pixel is 503.7628 m^2, to ha
class_stat$area <- (class_stat[,3]*503.7628)*0.0001

class_stat %>%
  filter(ctype %in% c("FST")) %>%
  ggplot(aes(x = Year, y = area, group = cnames , colour = cnames)) +
  labs(title = "Landsat Land Classification", x = "Year", y = "Area (ha)") +
  theme_minimal() +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  geom_line(linewidth = 1.2)

class_stat %>%
  filter(ctype %in% c("CRP","SHR","WET","IMP","BaA","Wat","PIS")) %>%
  ggplot(aes(x = Year, y = area, group = cnames , colour = cnames)) +
  labs(title = "Landsat Land Classification", x = "Year", y = "Area (ha)") +
  theme_minimal() +
  theme(legend.position = "bottom", legend.title = element_blank())  +
...
Nakazdli_sf_ext <- ext(Nakazdli_sf)

rstac::stac_search(
  q = stac_source,
  collections = "alos-fnf-mosaic",
  bbox = c(as.numeric(Nakazdli_sf_ext[1]),
           as.numeric(Nakazdli_sf_ext[2]),
           as.numeric(Nakazdli_sf_ext[3]),
           as.numeric(Nakazdli_sf_ext[4])),
  limit = 999
) |> 
  rstac::get_request()

# For some reason this requires individual
# years and only 2018-2020

startd <- c("2018-01-01", "2019-01-01","2020-01-01")
endd <- c("2018-12-31", "2019-12-31","2020-12-31")

for(i in 1:3){
alos_palsar_band_mapping <- get_stac_data(
  aoi = Nakazdli_sf[1],
  start_date = startd[i],
  end_date = endd[i],
  asset_names='C',
  ...
  #composite_function = NULL,
  output_filename = paste0(here::here("Outputs","Rasters","ALOS"),"/ALOS",i,".tif")
  )
}

ALOS18 <- rast(dir(paste0(here::here("Outputs","Rasters","ALOS")),
               full.names=TRUE,
               pattern="ALOS1.tif"))
ALOS19 <- rast(dir(paste0(here::here("Outputs","Rasters","ALOS")),
               full.names=TRUE,
               pattern="ALOS2.tif"))
ALOS20 <- rast(dir(paste0(here::here("Outputs","Rasters","ALOS")),
               full.names=TRUE,
               pattern="ALOS3.tif"))

ALOS20b <- terra::project(ALOS20, "EPSG:32610", threads=n.cores)

ALOS20b <- mask(ALOS20, Nakazdli)
...
EcoZonea <- vect( paste0(here("Inputs","Shapefiles"),
                   "/EcoZoneagg.shp"))
...
# Land Cover
#LandCovs <- as.factor(LandCov)

install.packages("wesanderson")
library(wesanderson)
names(wes_palettes)

wes_palette("Cavalcanti1", type = "continuous")

tm_shape(LandCovs) +
    tm_raster(palette = wes_palette("Cavalcanti1", type = "continuous")) +
tm_layout(legend.outside = FALSE) +
tm_layout(legend.position= c("right", "top")) +
tm_compass(type="arrow", position=c(0.2, 0.80))+
tm_scalebar(position = c(0.12, .1), text.size=.8) + tm_shape(st_as_sf(Montane))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Boreal))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Taiga))+
tm_borders(lwd=2)
...
Can3D_DEM <- rast(paste0(here("Inputs",
                              "Rasters",
                              "CDEM"),
                         "/Can3D_DEM.tif"))

Admin_bndry <- vect(paste0(here("Inputs",
                               "Shapefiles"),
                               "/AdminBoundaries_W.shp"))

Can3D_DEM <- crop(Can3D_DEM,
                  Admin_bndry,
                  mask=TRUE)



tm_shape(st_as_sf(Admin_bndry)) + tm_polygons() + tm_shape(Can3D_DEM) +
  tm_raster(palette = wes_palette("Zissou1", 21, type = "continuous")) +
tm_compass(type="arrow", position=c(0.4, 0.85))+
tm_scalebar(position = c(0.15, .1), text.size=.8) + tm_shape(st_as_sf(Montane)) +
tm_borders(lwd=2) + tm_shape(st_as_sf(Boreal))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Taiga))+
tm_borders(lwd=2)
...
plot(Can3D_DEM)
plot(Admin_bndry, add=TRUE)

ggplot(LandCovs) +
  geom_raster() +
  scale_fill_gradientn(colours = terrain.colors(length(values(LandCovs))))



plot(LandCovs,
     add=TRUE,
     col=wes_palette("Cavalcanti1", type = "continuous"),
     package='terra')
...
AMMA_map <- tm_shape(AMMA_RFmod) +
  tm_raster(style = "cont", palette = "RdYlGn",
            title = "AMMA SDM", midpoint = NA) +
tm_layout(legend.position= c("right", "top")) +
tm_compass(type="arrow", position=c(0.70, 1.00))+
tm_scalebar(text.size=.8, position=c(0.04, 0.08)) + tm_shape(st_as_sf(Montane))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Boreal))+
tm_borders(lwd=2)
...
RASY_map <- tm_shape(RASY_RFmod) +
  tm_raster(style = "cont", palette = "RdYlGn",
            title = "RASY SDM") +
tm_layout(legend.position= c("right", "top"),
				  inner.margins=c(0.04,0.22, .02, .01)) +
tm_compass(type="arrow", position=c(0.65, 1.00))+
tm_scalebar(text.size=.8, position=c(0.01, 0.07)) + tm_shape(st_as_sf(Montane))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Boreal))+
tm_borders(lwd=2) + tm_shape(st_as_sf(Taiga))+
tm_borders(lwd=2)
...
tmap_save(AMMA_map, filename = paste0(here("Outputs","Figures"),"/AB9(7)_Ch 3_Fig 3.4.jpeg"), height = 8.27, width = 11.69, dpi=600)
          
tmap_save(ANBO_map, filename = paste0(here("Outputs","Figures"),"/AB9(7)_Ch 3_Fig 3.5.jpeg"),height = 8.27, width = 11.69, dpi=600)
          
tmap_save(RALU_map, filename = paste0(here("Outputs","Figures"),"/AB9(7)_Ch 3_Fig 3.6.jpeg"),height = 8.27, width = 11.69, dpi=600)
          
tmap_save(RASY_map, filename = paste0(here("Outputs","Figures"),"/AB9(7)_Ch 3_Fig 3.7.jpeg"),height = 8.27, width = 11.69, dpi=600)
          
tmap_save(AMTI_map, filename = paste0(here("Outputs","Figures"),"/AB9(7)_Ch 3_Fig 3.8.jpeg"),height = 8.27, width = 11.69, dpi=600)
...
AMMA_clim <- rast(paste0(here("Inputs","Rasters","MaxEnt"),"/AMMA_climate.tif"))
ANBO_clim <- rast(paste0(here("Inputs","Rasters","MaxEnt"),"/ANBO_climate.tif"))
RALU_clim <- rast(paste0(here("Inputs","Rasters","MaxEnt"),"/RALU_climate.tif"))
RASY_clim <- rast(paste0(here("Inputs","Rasters","MaxEnt"),"/RASY_climate.tif"))

## Mapping it together

AMMA_map2 <- tm_shape(AMMA_RFmod) +
  tm_raster(style = "cont", palette = "RdYlGn", midpoint = NA) +
...
RF_SDM_maps <-  tmap_arrange(AMMA_map, AMTI_map, ANBO_map, RASY_map, RALU_map, ncol=3, nrow=2)

#Lee-Yaw and Irwin (2015) MaxEnt

AMMA_clim_LY1 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Lee_Yaw_allo"),
                         "/prediction1.tif"))
AMMA_clim_LY2 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Lee_Yaw_allo"),
                         "/prediction2.tif"))
AMMA_clim_LY3 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Lee_Yaw_allo"),
                         "/prediction3.tif"))
AMMA_clim_LY4 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Lee_Yaw_allo"),
                         "/prediction4.tif"))
AMMA_clim_LY5 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Lee_Yaw_allo"),
                         "/QGIS_prediction5.tif"))

AMMA_clim_LY5 <- terra::project(AMMA_clim_LY5, crs(AMMA_RFmod))
AMMA_clim_LY5 <- terra::mask(AMMA_clim_LY5,Boreal_Montane)
AMMA_clim_LY5 <- terra::crop(AMMA_clim_LY5,Boreal_Montane)

AMMA_clim_LY5_map <- tm_shape(AMMA_clim_LY5) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, main.title="Lee-Yaw and Irwin 2015, North Central Group", main.title.size=0.6) +
  tm_shape(st_as_sf(Boreal_Montane)) +
  tm_borders(lwd=2)

AMMA_map3 <- tm_shape(AMMA_RFmod) +
  tm_raster(style = "cont", palette = "RdYlGn", midpoint = NA) +
  tm_layout(legend.show= FALSE, main.title="Random Forest - This Chapter", main.title.size=0.6) +
  tm_shape(st_as_sf(Boreal_Montane)) +
  tm_borders(lwd=2)

AMMA_clim_map2 <- tm_shape(AMMA_clim) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, main.title="Seaborn et al., 2021",  main.title.size=0.6) +
  tm_shape(st_as_sf(Boreal_Montane)) +
  tm_borders(lwd=2)
AMMA_Pred_maps <-  tmap_arrange(AMMA_clim_LY5_map, AMMA_map3, AMMA_clim_map2, ncol = 3)

tmap_save(AMMA_Pred_maps, filename = paste0(here("Outputs","Figures"),"/ABx(x)_Ch 3_Figx.x_AMMA_MaxEnt_comp.jpeg"),height = 8.27, width = 11.69, dpi=300)

#Plethodon

PLVE_clim_LY1 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Pletho"),
                         "/P_idahoensis_raster.tif"))
PLVE_clim_50 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Pletho"),
                         "/P_idahoensis_miroc5_8550.tif"))
PLVE_clim_70 <- rast(paste0(here("Inputs","Rasters","MaxEnt","Pletho"),
                         "/P_idahoensis_miroc5_8570.tif"))

PLVE_clim_LY1 <- terra::project(PLVE_clim_LY1, crs(AMMA_RFmod))
PLVE_clim_LY1 <- terra::mask(PLVE_clim_LY1,Boreal_Montane)
PLVE_clim_LY1 <- terra::crop(PLVE_clim_LY1,Boreal_Montane)

PLVE_clim_50 <- terra::project(PLVE_clim_50, crs(AMMA_RFmod))
PLVE_clim_50 <- terra::mask(PLVE_clim_50,Boreal_Montane)
PLVE_clim_50 <- terra::crop(PLVE_clim_50,Boreal_Montane)

PLVE_clim_70 <- terra::project(PLVE_clim_70, crs(AMMA_RFmod))
PLVE_clim_70 <- terra::mask(PLVE_clim_70,Boreal_Montane)
PLVE_clim_70 <- terra::crop(PLVE_clim_70,Boreal_Montane)

PLVE_clim_map <- tm_shape(PLVE_clim_LY1) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, main.title="PLID Current",  main.title.size=1.5) +
  tm_shape(st_as_sf(Montane)) +
  tm_borders(lwd=2)

PLVE_clim_map50 <- tm_shape(PLVE_clim_50) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, main.title="PLID 2050",  main.title.size=1.5) +
  tm_shape(st_as_sf(Montane)) +
  tm_borders(lwd=2)

PLVE_clim_map70 <- tm_shape(PLVE_clim_70) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, main.title="PLID 2070",  main.title.size=1.5) +
  tm_shape(st_as_sf(Montane)) +
  tm_borders(lwd=2)

PLVE_clim_maps <-  tmap_arrange(PLVE_clim_map, PLVE_clim_map50, PLVE_clim_map70, ncol = 3)

tmap_save(PLVE_clim_maps, filename = paste0(here("Outputs","Figures"),"/ABx(x)_Ch 3_Figx.x_PLVE_clim_maps.jpeg"),height = 8.27, width = 11.69, dpi=300)


# AMontanusRF

AMont_SR <- rast(paste0(here("Inputs","Rasters","AMontanusRF"),"/w001001x.adf") )
