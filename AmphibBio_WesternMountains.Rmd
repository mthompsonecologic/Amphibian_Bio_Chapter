...
Taiga_MCMT <- rast(paste0(here::here("Inputs",
                                  ...
                                  "Explanatory"),
                                     "/Taiga_MCMT.tif"))
maskrasts <- function (x){
  apply(mask(x,EcoZone))
...
CDEM_boreal <- resample(CDEM_boreal,Rough_boreal)
CDEM_boreal <- mask(CDEM_boreal,Rough_boreal)

Boreal_MAT  <- resample(Boreal_MAT,Rough_boreal)
Boreal_MAT <- mask(Boreal_MAT,Rough_boreal)

Boreal_MCMT <- resample(Boreal_MCMT,Rough_boreal)
Boreal_MCMT <- mask(Boreal_MCMT,Rough_boreal)


SA_boreal <- resample(SA_boreal,Rough_boreal)
SA_boreal <- mask(SA_boreal,Rough_boreal)

NDVI_boreal <- resample(NDVI_boreal,Rough_boreal)
NDVI_boreal <- mask(NDVI_boreal,Rough_boreal)

LandCov_boreal <- resample(LandCov_boreal,Rough_boreal)
LandCov_boreal <- mask(LandCov_boreal,Rough_boreal)
...
CDEM_montane <- resample(CDEM_montane,Rough_montane)
CDEM_montane <- mask(CDEM_montane,Rough_montane)

Montane_MAT  <- resample(Montane_MAT,Rough_montane)
Montane_MAT <- mask(Montane_MAT,Rough_montane)

Montane_MCMT <- resample(Montane_MCMT,Rough_montane)
Montane_MCMT <- mask(Montane_MCMT,Rough_montane)

SA_montane <- resample(SA_montane,Rough_montane)
SA_montane <- mask(SA_montane,Rough_montane)

NDVI_montane <- resample(NDVI_montane,Rough_montane)
NDVI_montane <- mask(NDVI_montane,Rough_montane)

LandCov_montane <- resample(LandCov_montane,Rough_montane)
LandCov_montane <- mask(LandCov_montane,Rough_montane)
...
CDEM_taiga <- resample(CDEM_taiga,Rough_taiga)
CDEM_taiga <- mask(CDEM_taiga,Rough_taiga)

Taiga_MAT  <- resample(Taiga_MAT,Rough_taiga)
Taiga_MAT <- mask(Taiga_MAT,Rough_taiga)

Taiga_MCMT <- resample(Taiga_MCMT,Rough_taiga)
Taiga_MCMT <- mask(Taiga_MCMT,Rough_taiga)

SA_taiga <- resample(SA_taiga,Rough_taiga)
SA_taiga <- mask(SA_taiga,Rough_taiga)

NDVI_taiga <- resample(NDVI_taiga,Rough_taiga)
NDVI_taiga <- mask(NDVI_taiga,Rough_taiga)

LandCov_taiga <- resample(LandCov_taiga,Rough_taiga)
LandCov_taiga <- mask(LandCov_taiga,Rough_taiga)
