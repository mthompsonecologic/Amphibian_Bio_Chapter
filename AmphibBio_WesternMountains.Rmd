...
# MAT, elev, Rough, SA - importance
# rfcv dif between 7-5 low, so:
# go with three :
...
# elev, MAT, NDVI, LC
AMMA_rfm3 <-randomForest(PA~elev + MAT + NDVI + LC, data=AMMA_data)
print(AMMA_rfm3)
...
# SA, MAT, Rough, NDVI
ANBO_rfm3 <-randomForest(PA~SA + MAT + Rough + NDVI, data=ANBO_data)
print(ANBO_rfm3)
...
# Reduce to x variables:
# All but elevation
RALU_rfm3 <-randomForest(PA~MAT + NDVI + Rough + SA + LC + MCMT, data=RALU_data)
...
# Reduce to x variables:
# 
RALU_rfm3 <-randomForest(PA~MCMT+elev+Rough+MAT+SA, data=RALU_data)
print(RALU_rfm3)
...
# Reduce to x variables:
# All but Rough
RASY_rfm3 <-randomForest(PA~LC+NDVI+MAT+SA+elev+MCMT, data=RASY_data)
print(RASY_rfm3)
...
# Reduce to x variables:
# All but SA or LC
RASY_rfm3 <-randomForest(PA~elev+MCMT+MAT+Rough+NDVI+LC, data=RASY_data)
print(RASY_rfm3)
...
# Reduce to x variables:
# All but SA or LC
RASY_rfm3 <-randomForest(PA~MCMT+elev, data=RASY_data)
print(RASY_rfm3)
...
AMMA_map2 <- tm_shape(AMMA_RFmod) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, title="RF") +
  tm_shape(st_as_sf(Boreal_Montane)) +
  tm_borders(lwd=2)

AMMA_clim_map <- tm_shape(AMMA_clim) +
  tm_raster(style = "cont", palette = "RdYlGn") +
  tm_layout(legend.show= FALSE, title="MaxEnt") +
  tm_shape(st_as_sf(Boreal_Montane)) +
  tm_borders(lwd=2)
...
AMMA_point_map <-  tm_shape(st_as_sf(Boreal_Montane)) +
  ...
  tm_layout(legend.show= FALSE, title="Points") 

current.mode <- tmap_mode("plot")
tmap_arrange(AMMA_map2, AMMA_point_map, AMMA_clim_map)
...
current.mode <- tmap_mode("plot")
tmap_arrange(ANBO_map2, ANBO_point_map, ANBO_clim_map)
tmap_mode(current.mode)
...
current.mode <- tmap_mode("plot")
tmap_arrange(RASY_map2, RASY_point_map, RASY_clim_map)
tmap_mode(current.mode)
